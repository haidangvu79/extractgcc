...
void dynamic_buffer_underrun_031()
{
	...
	{
	    ptr1[11]='\0';
	    // Frama-C/Eva: the actual undefined behavior happens inside the
	    // loop body, not when decrementing/testing an integer
	    for(i=10;i>=-1;i--)
	    {
              ptr1[i]='a'; /*Tool should detect this line as error*/ /*ERROR:Buffer Underrun*//*FC-adjusted*/
	    }
	    memcpy(ptr2,ptr1,12);
	    free(ptr1);
	}
...
void dynamic_buffer_underrun_033()
{
    ...
    if(message !=NULL)
    {
	    while(len>=-2 )
	    {
		   // Frama-C/Eva: the actual undefined behavior happens when
		   // reading the buffer with an invalid offset
              c = message[len]; /*Tool should detect this line as error*/ /*ERROR:Buffer Underrun*//*FC-adjusted*/
		   if(isspace(c))
		   {
			   message[len]='\n';
		   }
...
void dynamic_buffer_underrun_035()
{
	...
	loc1--;
        ...
        // Frama-C/Eva: the actual undefined behavior happens in this access,
        // when loc1 == -1
	doubleptr[loc1][loc2]='T'; /*Tool should detect this line as error*/ /*ERROR:Buffer Underrun*//*FC-adjusted*/
...
void dynamic_buffer_underrun_039()
{
	...
	if(ptr_s1 != NULL)
	{
		for(i=-10;i<15;i++)
		{
			// Frama-C/Eva: there is no actual buffer underrun
			// here (i is never used), so we patched it in order
			// to have some undefined behavior
			//memset(ptr_s1,1,15*sizeof(dynamic_buffer_underrun_s_008));
			memset(ptr_s1+i,1,15*sizeof(dynamic_buffer_underrun_s_008)); /*Tool should detect this line as error*/ /*ERROR:Buffer Underrun*//*FC-adapted*/
		}
