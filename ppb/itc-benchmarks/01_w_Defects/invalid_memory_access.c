...
void invalid_memory_access_003 ()
{
  ...
  //Frama-C/Eva: the access of the indeterminate value below (c is dangling)
  //             is considered UB
  psink = c;
}
...
int invalid_memory_access_005 (int count)
{
	...
	if (count ==0)
	{
    	count = *ptr; /*Tool should detect this line as error*/ /*ERROR:Invalid memory access to already freed area*//*FC-disabled*/
		return 1;
	}
    else
    {
...
*/
// Frama-C/Eva: the test is incorrect; the line containing the error is
// unreachable. We changed the labels/gotos to fix it.
