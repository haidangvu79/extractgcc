...
void zero_division_008 ()
{
	...
	float ret;
	// Frama-C/Eva: floating-point division does not cause undefined
	// behavior, according to the standard; instead, following the
	// semantics of IEC 60559, it produces a NaN value.
	// Some tools may decide to report this as an error (and Frama-C does
	// so, for some values of option -warn-special-float), but it is
	// not undefined behavior.
	ret = dividend / 0.0;/*Tool should detect this line as error*/ /* ERROR:division by zero *//*FC-alarm-but-noUB*/
}
