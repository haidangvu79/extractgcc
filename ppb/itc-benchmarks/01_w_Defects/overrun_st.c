...
// Frama-C/Eva: several tests reference global variable 'idx' in "sink"
// statements. 'idx' is declared as a global and never written to, so it
// always contains 0. The "sink" statements are used presumably to prevent
// tools from generating warnings such as "variable never read" for the
// test arrays. However, some of the test arrays don't have their first
// element properly initialized, which makes the "sink" statement a possible
// source of undefined behavior unrelated to that being tested.
// Therefore, we remove such statements when compiling with Frama-C, since
// they are unnecessary for Frama-C/Eva.
...
void overrun_st_044 ()
{
	...
	for (i = 0; i <= 5; i ++)
	{
		// Frama-C/Eva: actual UB happens during pointer dereferencing
		*p = 1;/*Tool should detect this line as error*/ /*ERROR: buffer overrun *//*FC-adjusted*/
		p ++;
	}
#ifndef __FRAMAC__
        sink = buf[idx];
