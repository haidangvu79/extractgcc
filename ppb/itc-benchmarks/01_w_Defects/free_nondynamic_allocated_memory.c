...
{
	// Frama-C/Eva: the undefined behavior does not happen when assigning
	// the pointer, only when calling free() on it.
	free_nondynamic_allocated_memory_003_gbl_ptr = "STRING";
	free(free_nondynamic_allocated_memory_003_gbl_ptr); /*Tool should detect this line as error*/ /*ERROR:Free memory not allocated dynamically*//*FC-adjusted*/
}
...
void free_nondynamic_allocated_memory_013()
{
	...
	new_struct->next = &str;
	// Frama-C/Eva: this test was missing the error comment, which was
	// added below
	free(new_struct->next);/*Tool should detect this line as error*/ /*ERROR:Free memory not allocated dynamically*//*FC-adjusted*/
	free(new_struct);
}
