...
void memory_allocation_failure_005 ()
{
	...
	if(ret >= 0)
		if(vptr != NULL) {
			free(vptr);
			// Frama-C/Eva: the line below was added to avoid
			// issues with later tests which use vptr. By ensuring
			// that vptr does not point to dangling memory, we
			// avoid 'polluting' later tests.
			vptr = NULL;
                }
}
