...
void redundant_cond_009 ()
{
	...
	{
	// Frama-C/Eva: the lines below trigger undefined behavior: if a < 10,
	// the loop will execute until a becomes negative, then it will perform
	// a negative overflow before becoming positive and exiting the loop.
	// We add "if (a < 0) break;" to prevent UB from happening.
		b += a;
		a --;
		if (a < 0) break;
	}
...
void redundant_cond_013 ()
{
	...
	while ((a > 10))/*Tool should not detect this line as error*/ /*No ERROR:Redundant condition*/
	{
	// Frama-C/Eva: the loop body can lead to UB due to signed overflow:
	// b increases much faster than a reaches 10, leading to a possible
	// signed overflow. We add a check to prevent UB from happening.
		if (b > INT_MAX - RAND_MAX) break;
		b += a;
		a --;
	}
	ret = b;
...
void redundant_cond_014 ()
{
  ...
  do {
    ...
    // Frama-C/Eva: as noted in the comment above, the code below may cause
    // an overflow, and thus the cast to unsigned was inserted to prevent it.
    // However, there is still a possible overflow when storing the value in b,
    // which causes a "data overflow" as in the tests performed in other files.
    // If we want to be coherent, we must prevent the overflow from happening.
    // Otherwise, the tool should be guessing the intent of the test, in order
    // to decide whether to detect an error.
    // We add a check to prevent the overflow from happening.
    if (b > INT_MAX - a) break;
    b += (unsigned)a;
    a --;
  }
