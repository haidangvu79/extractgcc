...
void bit_shift_009 ()
{
	...
	int shift;
	int ret;
	// Frama-C/Eva: the test below does contain an undefined behavior:
	// left-shifting the most significant bit of a signed integer leads
	// to 1 << 31, which is not representable as a (signed) integer.
	// we patched it to avoid this UB.
	//shift = rand() % 32;
	shift = rand() % 31;
	ret = a << shift;/*Tool should  Not detect this line as error*/ /*NO ERROR:Bit shift error*/
        sink = ret;
