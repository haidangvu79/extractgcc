...
void uninit_memory_access_001 ()
{
	...
	unsigned long *ret;
	ret = &a;
        // Frama-C/Eva: fixed the format modifier below (%ld -> %lu) to
        // avoid undefined behavior
        printf("%lu ",*ret); /*Tool should not detect this line as error*/ /*No ERROR:Uninitialized Memory Access*/
}
...
void uninit_memory_access_010 ()
{
    ...
    if(uninit_memory_access_010_s_001_arr_gbl!=NULL)
    {
      // Frama-C/Eva: the storage location referred to by
      // uninit_memory_access_010_s_001_arr_gbl->data is volatile.
      // By default, Frama-C/Eva considers that access to volatile data,
      // _even after the same storage has been written in the program_,
      // can contain any value. For instance, the memory location could
      // represent a hardware register. In the C11 standard, 6.7.3.7,
      // we have: "An object that has volatile-qualified type may be modified
      // in ways unknown to the implementation or have other unknown side
      // effects."
      // In practical terms, this means that Frama-C/Eva considers that
      // the variable below might have value SHRT_MAX before the increment,
      // leading to a signed overflow and therefore UB.
      // We add a check (using a non-volatile variable) to prevent this from
      // happening; other tools which interpret the standard in a different
      // manner may not need this.
#ifdef __FRAMAC__
      unsigned short data = uninit_memory_access_010_s_001_arr_gbl->data;
      if (data < SHRT_MAX) {
        uninit_memory_access_010_s_001_arr_gbl->data = data + 1;
      }
#else
...
void uninit_memory_access_011_func_001 (const char *msg) {
  ...
  // Frama-C/Eva: currently it is not possible to infer that error_log, being
  // statically evaluated to "STRING", results in 'buffer' being properly
  // terminated. This leads to a yellow alarm in the next line, since
  // Frama-C/Eva believes the buffer might not have been correctly
  // nul-terminated.
  printf(" test %s\n", buffer);
}
