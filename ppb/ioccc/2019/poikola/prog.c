// NOTE for Frama-C: this file has been modified since the original '$' as
// label is non-portable and not parsed by Frama-C. We use 'aa' instead.
...
}if(H<z){ while       (H-- )    Z goto aa ;  }       if (z){ H -= z;               Q= P; while(z--       )Z c.K[F]^=B;    B=u=i; if(o-r==
...
aa: c.K[F]^=(B ^         ((V) ((V)(P | 1 <<          P)<< u* b) )) ;               Q^=Q; c.K[o-r-1          ]^=w[ ~ -o ]; goto*j[-~(P-P)]
