...
This is a subset of https://github.com/c-testsuite/c-testsuite, containing
only some files from the `tests/single-exec` directory,
plus `README.md` and `LICENSE` files.
The original contents of `README.md` are copied below.
The `LICENSE` file originally in `tests` has been renamed `LICENSE-tests`.
